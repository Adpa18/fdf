...
def frame(clock, w):
    ...
    sec = 1 / time1.seconds
    time1 = clock.restart()
    txt1 = sf.Text("Framerate :" + str(int(sec)))
...
def main():
    ...
    while w.is_open:
        ...
        frame(clock, w)
        if i < 200 or i > 220:
            tab[i].position = random.randint(-15, 15), random.randint(-15, 15)
        else:
            tab[i].position = random.randint(-15, 15), random.randint(-700, 15)
        w.draw(tab[i])
        w.display()
        if i == 325 or i == 0:
            i = 0
